...
static unsigned int __frame_to_index(frame_t frame)
{
    return (frame / FRAME_SIZE);
}

static frame_t __index_to_frame(size_t index)
{
    return (frame_t)(FRAME_SIZE * index);
}

static frame_t __phys_addr_to_frame(phys_addr_t addr)
{
    return (addr & FRAME_T_MAX);
}

static void __frame_set_available(phys_addr_t addr, size_t length)
{
...
static void __frame_set_used(phys_addr_t addr, size_t length)
{
    frame_t first_frame = __phys_addr_to_frame(addr);
    frame_t last_frame = __phys_addr_to_frame(addr + length - 1);
...
void frame_init(uint32_t _mmap_addr, uint32_t mmap_length)
{
    if(0 == _mmap_addr)
    ...
    __frame_set_used(
        (phys_addr_t)(&_kernel_start) - KERNEL_VBASE, (uintptr_t)(&_kernel_end) - (uintptr_t)(&_kernel_start));
}
