...
static inline page_directory_t* __get_page_directory(uintptr_t __attribute__((unused)) addr)
{
    return (page_directory_t*)0xFFFFF000;
}

static inline int __table_index(uintptr_t addr)
{
    return ((addr >> 22) & 0x3FF);
}

static inline int __page_index(uintptr_t addr)
{
    return ((addr >> 12) & 0x3FF);
}

static inline int __page_directory_table_is_present(page_directory_t* page_directory, int index)
{
    return (((uint32_t*)page_directory)[index] & PAGE_FLAG_PRESENT);
}

static inline page_table_t* __page_directory_get_table(page_directory_t* __attribute__((unused)) page_directory,
    int index)
{
...
static inline int __page_table_page_is_present(page_table_t* table, int index)
{
    return (((uint32_t*)table)[index] & PAGE_FLAG_PRESENT);
}

static inline frame_t __page_table_get_page(page_table_t* table, int index)
{
    return (((uint32_t*)table)[index] & 0xFFFFF000);
}

static inline void __page_table_set_page(page_table_t* table, int index, frame_t frame, int flags)
{
    ((uint32_t*)table)[index] = ((uint32_t)frame) | flags;
}

static inline void __tlb_flush_page(void* vaddr)
{
 ...
 */
int page_map(void* vaddr, frame_t phys)
{
    ...
    if(!__page_directory_table_is_present(page_directory, table_index)) {
        ...
        __page_directory_set_table(page_directory, table_index, (page_table_t*)frame, KPAGE_FLAGS);
