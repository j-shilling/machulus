...
char __idt_descriptor[6] = {
    0,
};

static inline uint64_t idt_entry(uint32_t offset, uint16_t selector, uint8_t flags)
{
    return ((uint64_t)(offset & 0xFFFF0000) << (48 - 16)) | ((uint64_t)(offset & 0x0000FFFF)) |
        ((uint64_t)(selector) << 16) | ((uint64_t)(flags) << 40) | (0x0000800000000000);
}
