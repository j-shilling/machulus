...
static inline block_header_t* __block_header(void* block)
{
    return (block_header_t*)block - 1;
}

static inline void* __block_addr(block_header_t* header)
{
    ...
    while(NULL != block) {
        if(!(block->size & 1) && (block->size >= size_aligned)) {
            ...
            if((block->size - size_aligned - header_size) > ALIGNMENT) {
                block_header_t* next = (block_header_t*)((char*)__block_addr(block) + size_aligned);
                next->prev = block;
