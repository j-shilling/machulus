...
void __ubsan_handle_type_mismatch_v1(struct ubsan_type_mismatch_data *data,
                                     void *ptr) {
  const char *type_check_kind = TypeCheckKinds[data->type_check_kind];
  ...
  if (!ptr) {
    ...
    __ubsan_printf(
        &data->loc,
        "%s misaligned address %p for type %s which requires %d byte alignment\n",
        type_check_kind, ptr, data->type->type_name, alignment);
  } else {
    __ubsan_printf(
        &data->loc,
